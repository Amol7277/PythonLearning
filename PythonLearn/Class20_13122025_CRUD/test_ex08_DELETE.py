...
def test_update_booking_details():
    ...
    response = requests.put(url=full_url, headers= headers, json=booking_Details_json)
    response_json = response.json()
    assert response.status_code == 200
    assert response_json["firstname"] == "Amol"
    assert response_json["lastname"] == "Browne"
    assert response_json["totalprice"] == 120
...
def test_delete_booking():
    token = test_get_token()
    book_id = test_Create_and_get_booking_id()
    path_url = "/booking/" + str(book_id)
    full_url = base_url + path_url
    print(full_url)
